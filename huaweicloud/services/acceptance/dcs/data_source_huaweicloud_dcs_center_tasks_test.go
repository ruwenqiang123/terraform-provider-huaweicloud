...
func TestAccDatasourceCenterTasks_basic(t *testing.T) {
	rName := "data.huaweicloud_dcs_center_tasks.test"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDatasourceCenterTasks_basic(),
...
func testAccDatasourceCenterTasks_basic() string {
...
data "huaweicloud_dcs_center_tasks" "time_filter" {
  start_time = %[1]s
  end_time   = %[2]s
}

output "time_filter_is_useful" {
  value = length(data.huaweicloud_dcs_center_tasks.time_filter.tasks) > 0
}
`, startTime, endTime)
}
